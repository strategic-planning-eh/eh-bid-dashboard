...
const YRS=(function(){var ys={};(BAD.both.bidlist||[]).forEach(function(r){if(r.year)ys[r.year]=1;});var a=Object.keys(ys).sort();return a.length?a[0]+'\u2013'+a[a.length-1]:'';})();
...
const DSVC={'Environmental Studies / EIA (impact assessment)':'دراسات بيئية / تقييم الأثر البيئي','Unclassified':'غير مصنّف','Environmental Services':'خدمات بيئية','Training':'تدريب','Env. Services':'خدمات بيئية','Env. Studies / EIA':'دراسات بيئية / تقييم الأثر'};
...
function rKPI(){$('kstrip').innerHTML=
 kc(k.total,t('Tenders tracked','المنافسات المتتبَّعة'),YRS)+
...
function rOverview(){
 ...
 <div class="grid g2">
   ...
   <div class="card"><h3>${t('2025 → 2026 trajectory','مسار 2025 → 2026')}</h3><div class="note">${t('Tender volume held up but win rate fell sharply — the central competitive concern.','صمد حجم المنافسات لكن نسبة الفوز انخفضت بحدّة — وهو الشاغل التنافسي الأبرز.')}</div>
     ${cardRow([[t('Tenders','المنافسات'),t2[2025].bids+' → '+t2[2026].bids,''],[t('Win rate','نسبة الفوز'),pct(t2[2025].win_rate)+' → '+pct(t2[2026].win_rate),t2[2026].win_rate<t2[2025].win_rate?'down':''],[t('Tendered value','قيمة المنافسات'),'SAR '+fmtM(t2[2025].pipeline)+' → '+fmtM(t2[2026].pipeline),''],[t('Avg bidders','متوسط المتقدمين'),t2[2025].avg_bidders+' → '+t2[2026].avg_bidders,'']])}</div>
 </div>`;
}
...
function rPipeline(){
 ...
 const wl=BA.winloss;
 const statusSeg=[{l:t('Awarded — EH','مُرساة ل EH'),v:wl.awarded_eh,c:C.green},{l:t('Awarded — other','مُرساة لغيره'),v:wl.awarded_other,c:C.red},{l:t('Not awarded','لم تتم الترسية'),v:wl.not_awarded,c:C.orange},{l:t('Cancelled','ملغاة'),v:wl.cancelled,c:C.soft},{l:t('In progress / pending','قيد التنفيذ / الانتظار'),v:wl.in_progress+wl.no_status,c:C.blue}];
...
function rWinloss(){
 ...
 <div class="grid g2">
  <div class="card"><h3>${t('Head-to-head outcome','النتيجة المباشرة')}</h3><div class="note">${t('Among decided tenders, EH vs the field.','بين المنافسات المحسومة، EH مقابل المنافسين.')}</div>
   ...
   <div style="margin-top:12px;font-size:12px;color:#6B7C86;text-align:center">${t('EH won ','فاز EH ب ')}<b style="color:#2E7D46">${wl.awarded_eh}</b>${t(' · lost ',' · وخسر ')}<b style="color:#C0504D">${wl.awarded_other}</b>${t(' of '+k.awarded+' decided tenders.',' من '+k.awarded+' منافسة محسومة.')}</div></div>
...
function renderPricing(){
 ...
 const cards=rows.map((r,i)=>{
   ...
   const oc=r.status=='Cancelled'?'<span class="tag" style="background:#8A99A3">'+t('CANCELLED','ملغاة')+'</span>':(r.won==null?'<span style="color:#5F7078;font-weight:700">'+t('PENDING','قيد الدراسة')+'</span>':(r.won?'<span class="tag" style="background:#2E7D46">'+t('EH WON','فاز EH')+'</span>':'<span class="tag" style="background:#C0504D">'+t('LOST','خسارة')+'</span>'));
   ...
   return `<div class="card" style="margin-bottom:10px;padding:0;overflow:hidden">
     <div tabindex="0" role="button" onkeydown="if(event.keyCode===13){this.click()}" onclick="togglePx(${i})" style="cursor:pointer;padding:12px 16px;display:flex;align-items:center;gap:14px;flex-wrap:wrap;background:${open?'#F7FAFC':'#fff'}">
       ...
       <div style="text-align:center"><div style="font-size:9px;color:#5F7078;text-transform:uppercase">${t('vs Low','مقابل الأدنى')}</div><div style="font-weight:700;font-size:12.5px;color:${gapcol}">${r.gap!=null?(r.gap>0?'+'+r.gap+'%':r.gap+'%'):'\u2014'}</div></div>
       <div style="min-width:78px;text-align:center">${oc}</div>
       <div style="font-size:13px;color:#5F7078;transform:rotate(${open?'90':'0'}deg);transition:.15s">\u25b8</div>
     </div>
 ...
 $('s-pricing').innerHTML=
 `<div class="sech">${t('Pricing intelligence','تحليل الأسعار')}</div><div class="secsub">${t('Every tender with a recorded bidder list — '+s.tenders+' in all. '+s.full+' show EH\'s price against the full field (the ranking stats below are drawn from these); the rest list the bidders with prices where disclosed and “undisclosed value” otherwise. '+s.total_bidders+' disclosed bids across '+s.total_names+' bidder entries. Select any tender for the full list.','كل منافسة لها قائمة متقدمين مُسجّلة — '+s.tenders+' إجمالاً. '+s.full+' منها تُظهر سعر EH مقابل كامل المنافسين (وتُشتق إحصاءات الترتيب أدناه منها)؛ والباقي يسرد المتقدمين بالأسعار حيث أُعلنت وب«قيمة غير معلنة» فيما عدا ذلك. '+s.total_bidders+' عرضاً مُعلناً عبر '+s.total_names+' مُدخل متقدم. اختر أي منافسة لعرض القائمة الكاملة.')}</div>
 <div class="kstrip">
   ${kc(s.cheapest_pct+'%',t('Tenders where EH was cheapest','منافسات كان EH الأرخص فيها'),s.cheapest+t(' of ',' من ')+s.full+t(' full',' كاملة'),s.cheapest_pct>=40?'g':'r')}
...
function rClients(){
 ...
 <div class="card"><h3>${t('Top clients by tracked value','أبرز العملاء حسب القيمة المتتبَّعة')}</h3><div class="note">${t('Sum of EH offer values per client (partial where the tracker is not filled). '+(k.biggest_client?k.biggest_client+' is the largest client by value.':'')+'','مجموع قيم عروض EH لكل عميل (جزئي حيث لم يُملأ الجدول). '+(k.biggest_client?k.biggest_client+' أكبر عميل من حيث القيمة.':''))}</div>
...
function rService(){
 ...
 <div class="card"><h3>${t('Tenders by platform','المنافسات حسب المنصة')}</h3><div class="note">${t('Procurement channels — Etimad is the Saudi government e-tendering portal.','قنوات الشراء — «اعتماد» منصة المنافسات الحكومية السعودية.')}</div>
...
function rFunnel(){
 ...
 <div class="grid g2">
  ...
  <div class="card"><h3>${t('Bank-guarantee exposure','انكشاف الضمان البنكي')}</h3><div class="note">${t('What \"guarantee exposure\" means.','ماذا يعني «انكشاف الضمان».')}</div>
   <div style="text-align:center;padding:6px 0 10px"><div style="font-size:34px;font-weight:800;color:#7B6BA8;line-height:1">${guarFmt}</div><div style="font-size:11px;color:#5F7078;text-transform:uppercase;letter-spacing:.5px;margin-top:4px">${t('total bid-bond value issued','إجمالي قيمة سندات العطاء المُصدرة')}</div></div>
...
function rLimitations(){
 const lim=[
  ...
  [t('2025 vs 2026 not comparable','2025 مقابل 2026 غير قابلة للمقارنة'),t('2026 is a partial, in-progress year — many tenders are still pending and outcomes lag launch by months. A near-complete 2025 is being compared against a half-finished 2026 where wins may simply not be recorded yet. The same depresses the most recent months of any time view.','2026 سنة جزئية جارية — كثير من المنافسات ما زالت معلّقة والنتائج تتأخّر أشهراً عن الطرح. تُقارَن 2025 شبه المكتملة ب2026 نصف المنجزة حيث قد لا تكون الانتصارات مُسجّلة بعد. والأمر ذاته يخفض أحدث الأشهر في أي عرض زمني.')],
  ...
  [t('Competitor / client counts undercount','أعداد المنافسين / العملاء أقل من الحقيقة'),t('The Arabic↔English name reconciliation matches the major recurring rivals and big clients well, but many one-off Arabic-named SMEs go unmatched. "Competitors faced" counts matched competitors only; the real field is larger, and some small clients may be split across name variants.','مطابقة الأسماء عربي↔إنجليزي تُطابق المنافسين المتكررين الكبار والعملاء الكبار جيداً، لكن كثيراً من المنشآت الصغيرة بأسماء عربية لمرة واحدة تبقى دون مطابقة. «المنافسون المواجَهون» يعدّ المطابقين فقط؛ فالميدان الحقيقي أكبر، وقد ينقسم بعض العملاء الصغار عبر تنويعات الاسم.')],
...
function rChrome(){var el;
 ...
 if(el=$('h-sub'))el.textContent=t('Environmental Horizons (Afaq Al-Biah) — competitive bid analytics, tracking years '+YRS,'آفاق البيئة (Afaq Al-Biah) — تحليلات تنافسية للمنافسات، سنوات التتبّع \u2066'+YRS+'\u2069');
...
const ANSF={
  ntfstat(){
    var snap=null; try{snap=JSON.parse(localStorage.getItem('ehb.snap'));}catch(e){}
    var per= snap
      ? '\u2705 Personal engine: snapshot saved <b>'+(snap.when||'?')+'</b> covering '+Object.keys(snap.t||{}).length+' tenders. Unseen right now: '+NTF.nw.size+' new \u00b7 '+NTF.dec.size+' decisions \u00b7 '+NTF.upd.size+' updates \u00b7 '+NTF.px.size+' pricing.'
      : '\u26a0\ufe0f Personal engine: no snapshot in this browser yet \u2014 first visit here; flags start from the next data change.';
    var id='ntfdiag'+Date.now();
    setTimeout(function(){
      var el=document.getElementById(id); if(!el) return;
      Promise.all([
        fetch('changelog.json',{cache:'no-store'}).then(r=>r.ok?r.json():null).catch(()=>null),
        fetch('state.json',{cache:'no-store'}).then(r=>r.ok?r.json():null).catch(()=>null)
      ]).then(function(res){
        var lg=res[0], st=res[1];
        var shared= lg
          ? '\u2705 Shared pipeline: changelog reachable \u2014 <b>'+(lg.entries||[]).length+'</b> logged change(s), last run '+(lg.updated||'?')+(st?'; state covers '+Object.keys(st.t||{}).length+' tenders.':'.')
          : '\u274c Shared pipeline: changelog.json not reachable. On a local file this is expected \u2014 test on the hosted hub. If hosted: no green workflow run has produced output yet (ask the hub administrator to run the publishing workflow once).';
        el.innerHTML=shared+'<span dir="rtl" style="display:block;margin-top:5px;opacity:.8">'+(lg?'\u0627\u0644\u0645\u0646\u0638\u0648\u0645\u0629 \u0627\u0644\u0645\u0634\u062a\u0631\u0643\u0629 \u062a\u0639\u0645\u0644.':'\u0627\u0644\u0645\u0646\u0638\u0648\u0645\u0629 \u0627\u0644\u0645\u0634\u062a\u0631\u0643\u0629 \u063a\u064a\u0631 \u0645\u062a\u0627\u062d\u0629 \u0645\u0646 \u0647\u0646\u0627.')+'</span>';
      });
    },80);
    return '<div class="ehm">'+per+'<div id="'+id+'" style="margin-top:7px;padding-top:7px;border-top:1px solid rgba(128,148,160,.25)">\u23f3 Checking the shared pipeline\u2026</div>'+
      '<span dir="rtl" style="display:block;margin-top:6px;opacity:.8">\u0641\u062d\u0635 \u0630\u0627\u062a\u064a \u0644\u0646\u0638\u0627\u0645 \u0627\u0644\u0625\u0634\u0639\u0627\u0631\u0627\u062a.</span></div>';
  },
  open(){
...
const QKW={
 ...
 open:['open pipeline','pipeline','pending','open tenders','still open','in progress','مفتوح','معلق','معلقه','قائمه','جاري'],
  ...
  ntfstat:['notification','notifications','badges','alerts','working','self test','selftest','اشعار','اشعارات','تنبيه','تنبيهات','شارات','يعمل'],
