 ...
 const cards=rows.map((r,i)=>{
   ...
   return `<div class="card" style="margin-bottom:10px;padding:0;overflow:hidden">
     <div onclick="togglePx(${i})" style="cursor:pointer;padding:12px 16px;display:flex;align-items:center;gap:14px;flex-wrap:wrap;background:${open?'#F7FAFC':'#fff'}">
       ...
       <div style="text-align:right"><div style="font-size:9px;color:#9AA8B0;text-transform:uppercase;letter-spacing:.3px">${t('EH bid','عرض EH')}</div><div style="font-weight:700;font-size:12.5px;color:#1A5FAB">${r.eh!=null?fmtSAR(r.eh):(r.bidders&&r.bidders.some(b=>b.eh)?'<span style="color:#7A93B5;font-weight:600;font-size:10px;font-style:italic">'+t('bid submitted — undisclosed','عرض مقدَّم — غير معلن')+'</span>':'<span style="color:#B0B8BD;font-weight:600;font-size:10px">'+t('no EH bid','لا عرض من EH')+'</span>')}</div></div>
