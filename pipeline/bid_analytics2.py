...
def compute(bids, rfull, comp):
    ...
    def platnorm(p):
        ...
        # SEC-SAP and SAP Ariba/Arabia are the same procurement channel (client confirmation, Jul 2026)
        if 'ariba' in pl or 'arabia' in pl and 'sap' in pl or 'sec' in pl or 'sap' in pl: return 'SEC-SAP / SAP Ariba'
    ...
    # The tracker's bidder count includes EH itself (verified against rosters), so
    # competition intensity = rivals = nbid - 1, grouped into bands for honest sample sizes.
    bd_dist=collections.defaultdict(lambda:{'count':0,'won':0,'awarded':0})
    _RVB=[('0',0,0),('1\u20132',1,2),('3\u20135',3,5),('6\u20139',6,9),('10+',10,999)]
    for b in bids:
        if not b['nbid']: continue
        rv=max(int(b['nbid'])-1,0)
        lab=next(l for l,a,z in _RVB if a<=rv<=z)
        d=bd_dist[lab]; d['count']+=1
        if b['decided']: d['awarded']+=1; d['won']+=1 if b['eh_won'] else 0
    _rvo={l:i for i,(l,_,_) in enumerate(_RVB)}
    bidder_dist=[dict(n=k, count=v['count'], won=v['won'], awarded=v['awarded']) for k,v in sorted(bd_dist.items(), key=lambda kv:_rvo[kv[0]])]
    ...
    def _na(x):
        ...
        return t
    price_rows=[]; ranks=[]; gaps=[]; wgaps=[]; cheapest_cnt=0
    ...
    for key,bidders in rfull.items():
        ...
        # gap of EH's price to the bid that actually WON (competitor wins only)
        if ehp is not None and award and bb and not bb['eh_won'] and wc and wc!='__EH__':
            wgaps.append(round(100*(ehp-award)/award,1))
    ...
    pricing=dict(rows=price_rows, summary=dict(
        ...
        median_win_gap=(round(st.median(wgaps),1) if wgaps else None), win_gap_n=len(wgaps),
    ...
    if 2025 in traj and 2026 in traj and traj[2025]['win_rate'] and traj[2026]['win_rate'] is not None and traj[2026]['win_rate']<traj[2025]['win_rate']:
...
import build_competitors as _BC
def scope(y=None):
    B=[b for b in _ALLBIDS if y is None or b['year']==y]
    RF={k:v for k,v in _ALLR.items() if y is None or int(k.split('-')[0])==y}
    comp,_wc=_BC.build(B, RF)
    return compute(B, RF, comp)

OUT=dict(both=scope(), y2025=scope(2025), y2026=scope(2026))
json.dump(OUT, open('bidanalytics2.json','w'), ensure_ascii=False, indent=1)

for sc in ('both','y2025','y2026'):
    o=OUT[sc]
    print(sc, '| KPIs:', {k:o['kpi'][k] for k in ['total','awarded','eh_won','win_rate']},
          '| platforms:', [(p['name'],p['count']) for p in o['platform_mix']],
          '| intensity:', [(d['n'],d['won'],d['awarded']) for d in o['bidder_dist']],
          '| pricing:', o['pricing']['summary']['tenders'],'rows (',o['pricing']['summary']['partial'],'partial )')
