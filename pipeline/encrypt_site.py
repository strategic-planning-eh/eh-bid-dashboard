...
decrypts with AES-256-GCM. Nothing readable is ever on the public URL.
  ...
  • The hub unlocks once; the derived key is kept in sessionStorage (and in localStorage when the viewer ticks
    "remember this device"), so the tab pages, which are separate encrypted files inside iframes, open without asking.
  • A decrypted page installs a small fetch() shim before its own scripts run, so its calls for changelog.json,
    gov_news.json, state.json, … receive the decrypted JSON.
...
UNLOCK = r"""<!DOCTYPE html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1">
...
<script>
(function(){
const E=JSON.parse(document.getElementById('enc').textContent), SK='eh.hub.k', enc=new TextEncoder(), dec=new TextDecoder();
...
function render(html){const i=html.search(/<head[^>]*>/i);const at=i>=0?html.indexOf('>',i)+1:0;const out=html.slice(0,at)+PRELUDE+html.slice(at);
 document.open();document.write(out);document.close();}
async function boot(){const s=stored();if(s){try{render(await tryRaw(u(s)));return;}catch(e){forget();}}document.body.classList.remove('quiet');document.getElementById('p').focus();}
document.getElementById('f').addEventListener('submit',async ev=>{ev.preventDefault();const p=document.getElementById('p'),btn=document.getElementById('b'),err=document.getElementById('e');
 if(!p.value)return;btn.disabled=true;err.textContent='';try{const raw=await derive(p.value);const html=await tryRaw(raw);keep(raw,document.getElementById('r').checked);render(html);}
    ...
    for root, dirs, files in os.walk(site):
        ...
        for fn in files:
            ...
            if low.endswith('.html'):
                raw = open(p, 'rb').read()
                if raw.lstrip()[:60].find(b'id="enc"') >= 0 or b'EHENC1' in raw[:200]: continue   # already sealed
                iv, ct = seal(key, raw)
